...
void CRYPTO_encrypt_block(const BLOCK *block_to_encrypt,
                          ENCRYPTED_BLOCK *encrypted_block_output) {
  ...
  uint8_t outbuffer[sizeof(BLOCK)];
  ...
  const size_t num_bytes_to_encrypt = sizeof(BLOCK);
  ...
  uint8_t *tag_ptr = ciphertext_ptr + sizeof(BLOCK);

  memcpy(iv_ptr, iv, AES_IV_SIZE_BYTES);
  memcpy(ciphertext_ptr, outbuffer, sizeof(BLOCK));
...
void CRYPTO_decrypt_block(const ENCRYPTED_BLOCK *block_to_decrypt,
  ...
  const uint8_t *tag_src_ptr = ciphertext_ptr + sizeof(BLOCK);
  ...
  const size_t num_bytes_to_decrypt = sizeof(BLOCK);
  ...
  memcpy(decrypted_block_output, outbuffer, sizeof(BLOCK));
